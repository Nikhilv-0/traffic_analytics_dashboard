# RoadPulse — shared extensions + database models.
#
# db/login_manager live here rather than in a separate extensions.py:
# models.py never needs to import app.py, so there's no circular-import
# risk — app.py, routes.py, and services.py all just import them from
# here instead.
#
...
#   - admin.js incident queue       -> Incident.status
#   - dashboard.js Congestion Forecast panel -> Prediction
#   - admin.js sessions panel + audit trail  -> Log
...
class Incident(db.Model):
    ...
    # (no 'resolved' state — once an admin's done with an incident, it's
    # either verified/rejected, or deleted outright via the admin panel)
    ...
    def to_dict(self):
        return {
            ...
            # see routes.py's _save_photo(). None when no photo was attached.
