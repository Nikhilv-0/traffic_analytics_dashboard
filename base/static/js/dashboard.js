...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('qa-report').addEventListener('click', () => {
        if (incidents.length === 0) {
            alert('No incident data to export yet.');
            return;
        }

        const headers = ['ID', 'Type', 'Severity', 'Location', 'Status', 'Reported At'];
        const csvEscape = (val) => {
            const str = String(val ?? '');
            return /[",\n]/.test(str) ? `"${str.replace(/"/g, '""')}"` : str;
        };
        const rows = incidents.map((inc) => [
            inc.id, inc.type, inc.severity, inc.location, inc.status, inc.reported_at
        ]);
        ...
        link.href = url;
        link.download = `roadpulse-incidents-${stamp}.csv`;
